from not_binary_node import DTNode
import numpy as np
import pandas as pd
...
class DecisionTreeClassifier:
    ...
    def get_best_split(self, dataset:DataFrame, num_features:int) -> dict:
        ...
        for feature_index in range(num_features):
            ...
            if type(feature_values[0]) is int: 
                split = self.split_continuo(dataset, feature_name, feature_values)
            else: 
    ...
    def split_continuo(self, dataset, feature_name, feature_values):
        best_division = None
        max_info_gain = -float("inf")
    ...
    def split_discreto(self, dataset, feature_name, feature_values):
        children = []
        for value in feature_values:
            children.append(dataset.query(f"{feature_name} == {value}"))
